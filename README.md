...
Backend solution explanation
----------------------------
I've taken the SpringBoot approach. 

To make it simple to test, the Angular app is built into public resources of the boot application and therefore accessible on the same port.
So it is important to build client side code before building the backend code.

The database is generated by liquibase migrations, and separate database for integration tests is used.  
 
As there is no specific security specified in the client code, so i've configured basic authentication which will bring
login dialog when needed.
 
There are 4 predefined users that will have their own expense lists:
  - rod/rod
  - jane/jane
  - freddy/freddy
  - zac/zac
 
Basic health monitoring is exposed and can be seen at:
- http://localhost:8080/metrics
- http://localhost:8080/health
    
I've implemented all the stories. Also added support for some extra currencies (AUD, USD, HUF...).

To fetch the exchange rate for any date i've used fixer.io api (http://fixer.io/). To call the service I have used OpenFeign library https://github.com/OpenFeign/feign 

The expenses are saved along with the original currency amount and the exchange rate used. I'm not allowing expenses in the future. I thought that makes sense.  

Backend requirements
--------------------
- java 8
- maven
- mysql with two schemas. By default user expense/expense in schemas expense and expense_test are configured.

Starting the service
--------------------
- clone repo, for this branch:
    - git clone git@github.com:zeljkokunica/backend-coding-challenge.git 
    - cd backend-coding-challenge 
    - git checkout zeljkokunica
- cd backend-coding-challenge
- npm install
- gulp dev
- cd expense-service
- open src/main/resources/application.properties and set the db properties.
- open src/main/resources/application-test.properties and set the db testing properties.
- mvn clean package
- java -jar target/expense-0.0.1-SNAPSHOT.jar --server.port=8080
- navigate to http://localhost:8080 and use any of the predefined credentials to authenticate
